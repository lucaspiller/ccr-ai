...
from .data_types import (ACTION_TYPE_OFFSETS, BOARD_HEIGHT, BOARD_WIDTH,
                         OFFSET_TO_ACTION_TYPE, TOTAL_TILES, ActionInfo)
...
def validate_action_space() -> bool:
    ...
    for action_idx in range(700):
        ...
        reconstructed_idx = encode_action(
            action_info.action_type, action_info.tile_row, action_info.tile_col
        )
        assert (
    ...
    for row in range(BOARD_HEIGHT):
        for col in range(BOARD_WIDTH):
            ...
            assert (reconstructed_row, reconstructed_col) == (
                row,
                col,
            ), f"Coordinate round trip failed for ({row}, {col})"
