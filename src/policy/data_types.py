...
class ActionInfo:
    ...
    def __post_init__(self):
        ...
        if self.tile_idx != expected_tile_idx:
            raise ValueError(f"tile_idx {self.tile_idx} doesn't match coordinates ({self.tile_row}, {self.tile_col})")
...
class PolicyOutput:
    ...
    def __post_init__(self):
        ...
        if self.action_logits.dim() != 1 or self.action_logits.size(0) != 700:
            raise ValueError(f"action_logits must be 1D with 700 elements, got {self.action_logits.shape}")
        
        if self.action_probs.dim() != 1 or self.action_probs.size(0) != 700:
            raise ValueError(f"action_probs must be 1D with 700 elements, got {self.action_probs.shape}")
        ...
        if self.entropy is None:
            ...
            log_probs = torch.log(self.action_probs + 1e-8)  # Add small epsilon for numerical stability
    ...
    def get_top_k_actions(self, k: int = 5) -> list[Tuple[int, float]]:
        ...
        return [(idx.item(), prob.item()) for idx, prob in zip(top_k_indices, top_k_values)]
...
assert TOTAL_ACTIONS == 560 + 140, f"Expected 700 actions (560 + 140), got {TOTAL_ACTIONS}"
assert max(ACTION_TYPE_OFFSETS.values()) + TOTAL_TILES <= TOTAL_ACTIONS, "Action space overflow"