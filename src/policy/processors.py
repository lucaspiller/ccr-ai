...
class PolicyHead(nn.Module):
    ...
    def __init__(self, config: Optional[PolicyConfig] = None):
        ...
        self.dropout = nn.Dropout(self.config.dropout_rate) if self.config.dropout_rate > 0 else None
    ...
    def get_action_probabilities(self, fused_embedding: torch.Tensor, temperature: Optional[float] = None) -> torch.Tensor:
        """
...
class PolicyProcessor:
    ...
    def forward(self, fused_embedding: torch.Tensor, 
                ...
                action_mask: Optional[torch.Tensor] = None) -> PolicyOutput:
        """
    ...
    def select_action(self, fused_embedding: torch.Tensor,
        ...
        if strategy == "deterministic":
            ...
            raise ValueError(f"Unknown strategy: {strategy}. Must be one of: deterministic, categorical, top_k")
    ...
    def select_batch_actions(self, fused_embeddings: torch.Tensor,
                            strategy: str = "deterministic",
                            temperature: Optional[float] = None,
                            top_k: Optional[int] = None,
                            action_masks: Optional[torch.Tensor] = None) -> List[PolicyOutput]:
        """
        ...
        for i in range(batch_size):
            ...
            result = self.select_action(
                embedding, strategy, temperature, top_k, mask
            )
    ...
    def _validate_input(self, fused_embedding: torch.Tensor) -> None:
        ...
        if fused_embedding.dim() not in [1, 2]:
            raise ValueError(f"fused_embedding must be 1D or 2D, got {fused_embedding.dim()}D")
        
        expected_dim = self.config.input_dim
        if fused_embedding.dim() == 1:
            if fused_embedding.size(0) != expected_dim:
                raise ValueError(f"Expected embedding size {expected_dim}, got {fused_embedding.size(0)}")
        else:  # 2D
            if fused_embedding.size(1) != expected_dim:
                raise ValueError(f"Expected embedding size {expected_dim}, got {fused_embedding.size(1)}")
    ...
    def _validate_action_mask(self, action_mask: torch.Tensor) -> None:
        ...
        if action_mask.dim() != 1 or action_mask.size(0) != self.config.output_dim:
            raise ValueError(f"action_mask must be 1D with {self.config.output_dim} elements")
...
def select_action_simple(fused_embedding: torch.Tensor, 
                        config: Optional[PolicyConfig] = None) -> Tuple[int, ActionInfo]:
    """
