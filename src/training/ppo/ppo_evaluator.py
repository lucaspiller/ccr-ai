...
from .puzzle_config import PuzzleConfig, SpriteConfig, WallConfig, PuzzleSpec
...
class PPOEvaluator:
    ...
    def __init__(
        ...
        self.evaluation_puzzles: Dict[str, List[PuzzleConfig]] = (
            self._generate_evaluation_puzzles()
        )
    ...
    def _generate_evaluation_puzzles(self) -> Dict[str, List[PuzzleConfig]]:
        """Generate fixed evaluation puzzle sets."""
        import random

        rng = random.Random(12345)  # Fixed seed for reproducibility

        puzzles: Dict[str, List[PuzzleConfig]] = {"easy": [], "medium": [], "hard": []}

        # Easy puzzles (100 puzzles)
        easy_spec = PuzzleSpec(
            name="Easy",
            width_range=(5, 8),
            height_range=(5, 6),
            mice_range=(1, 5),
        ...
        for i in range(100):
            puzzle = self._generate_puzzle_config(easy_spec, rng)
            puzzle.puzzle_id = f"easy_{i:03d}"
            puzzle.difficulty = "easy"
            puzzles["easy"].append(puzzle)

        # Medium puzzles (200 puzzles)
        medium_spec = PuzzleSpec(
            name="Medium",
            width_range=(8, 10),
            height_range=(6, 8),
            mice_range=(5, 7),
        ...
        for i in range(200):
            puzzle = self._generate_puzzle_config(medium_spec, rng)
            puzzle.puzzle_id = f"medium_{i:03d}"
            puzzle.difficulty = "medium"
            puzzles["medium"].append(puzzle)

        # Hard puzzles (200 puzzles)
        hard_spec = PuzzleSpec(
            name="Hard",
            width_range=(10, 14),
            height_range=(8, 10),
            mice_range=(7, 10),
        ...
        for i in range(200):
            puzzle = self._generate_puzzle_config(hard_spec, rng)
            puzzle.puzzle_id = f"hard_{i:03d}"
            puzzle.difficulty = "hard"
            puzzles["hard"].append(puzzle)

        return puzzles

    def _generate_puzzle_config(self, spec: PuzzleSpec, rng) -> PuzzleConfig:
        ...
        engine = level.create_engine(
            max_steps=self.config.execution_timeout, puzzle_mode=True
        )
        engine.start_game()
        ...
        while (
            ...
            and engine_steps < self.config.execution_timeout
        ):
            engine.step()
            engine_steps += 1
        ...
        if engine.result == GameResult.SUCCESS:
            ...
            return self._generate_puzzle_config(spec, rng)
    ...
    def evaluate(
        ...
        for difficulty in difficulties:
            ...
            print(f"Evaluating {len(difficulty_puzzles)} {difficulty} puzzles...")
        ...
        with torch.no_grad():
            ...
            while not episode_done and steps < self.config.placement_timeout:
                ...
                if torch.isnan(policy_logits).any() or torch.isinf(policy_logits).any():
                    print(f"    ERROR: NaN/Inf in policy_logits at step {steps}")
                    print(
                    ...
                    print(f"    observation has NaN: {torch.isnan(observation).any()}")
                    break
                ...
                if valid_actions == 0:
                    print(f"    ERROR: No valid actions available at step {steps}")
                    break
                ...
                if torch.isnan(masked_logits).any():
                    print(f"    ERROR: NaN in masked_logits at step {steps}")
                    print(
                ...
                for i, (idx, value) in enumerate(zip(top_indices, top_values)):
                    ...
                    print(f"      {i+1}. {desc} (logit={value:.3f}) {selected}")
                ...
                if step_result.info.get("phase") == "transition_to_execution":
                    ...
                    print(f"    Running game execution...")
                ...
                if not action_mask.any():
                    if not episode_done:
                        print(f"    No valid actions remaining at step {steps}")
                    break
            ...
            if len(action_history) > 5:
                ...
                print(f"    Action summary: {placements} placements, {erases} erases")
    ...
    def quick_evaluation(self, num_puzzles: int = 20) -> Dict[str, Any]:
        ...
        print(f"Running quick evaluation with {num_puzzles} puzzles per difficulty...")
