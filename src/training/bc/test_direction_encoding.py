...
def test_action_decoding_with_validation_board():
    ...
    for rank, action_id in enumerate(top_indices[0]):
        ...
        print(f"{rank+1:4d} | {action_id.item():9d} | {x:1d} | {y:1d} | {action_type:10s} | {'YES' if valid_pos else 'NO'}")
...
def test_label_creation_roundtrip():
    ...
    bfs_str_clean = bfs_str.replace('DOWN', '"DOWN"').replace('LEFT', '"LEFT"').replace('UP', '"UP"').replace('RIGHT', '"RIGHT"')
    ...
    for action_id in positive_indices:
        ...
        print(f"Action ID {action_id.item()} → ({action_info.x}, {action_info.y}, {action_info.action_type})")
    ...
    # Check if they match
    original_tuples = [(x, y, f"place_{d.name.lower()}") for x, y, d in converted_solution]
...
def test_direction_ordering_consistency():
    ...
    for direction in directions:
        try:
            ...
            print(f"{direction.name:9s} | {direction.value} | {action_id:9d} | {decoded_type:15s} | {'YES' if consistent else 'NO'}")
