...
class FusedStateOutput:
    ...
    def __post_init__(self):
        ...
        if self.fused_embedding.dim() != 1:
            raise ValueError(f"fused_embedding must be 1D, got {self.fused_embedding.dim()}D")
        
        if self.fused_embedding.size(0) != 128:
            raise ValueError(f"fused_embedding must have 128 dimensions, got {self.fused_embedding.size(0)}")
...
FUSION_INPUT_DIM = (GRID_TENSOR_CHANNELS * BOARD_HEIGHT * BOARD_WIDTH) + GLOBAL_FEATURES_DIM + CAT_EMBEDDING_DIM
