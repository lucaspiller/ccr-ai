        ...
        for module in self.modules():
            if isinstance(module, nn.Linear):
                if self.config.weight_init == "xavier":
                    ...
                    nn.init.kaiming_uniform_(module.weight, nonlinearity=self.config.activation)
...
class StateFusionProcessor:
    ...
    def fuse_batch(self, perception_outputs: list[PerceptionOutput]) -> list[FusedStateOutput]:
        """
    ...
    def fuse_batch_tensor(self, perception_outputs: list[PerceptionOutput]) -> torch.Tensor:
        """
        ...
        """
        embeddings = [self.fuse(output).fused_embedding for output in perception_outputs]
