...
class TestFusionConfig:
    ...
    def test_invalid_dropout_rate(self):
        ...
        with pytest.raises(
            ValueError, match="dropout_rate must be between 0.0 and 1.0"
        ):
    ...
    def test_empty_hidden_dims(self):
        ...
        with pytest.raises(
            ValueError, match="hidden_dims must contain at least one layer size"
        ):
...
class TestFusedStateOutput:
    ...
    def test_invalid_embedding_shape(self):
        ...
        # Wrong size
        with pytest.raises(
            ValueError, match="fused_embedding must have 128 dimensions"
        ):
...
class TestDifferentConfigurations:
    ...
    def test_with_without_layer_norm(self):
        ...
        assert (
            processor_with_ln.get_parameter_count()
